...
export default function CreateFolderModal({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await createFolder({ name, organizationId: initialOrgId });
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {organizations.length === 0 && (
              <div className="rounded-md bg-amber-50 p-3 text-sm text-amber-600 dark:bg-amber-900/20 dark:text-amber-300">
                Create an organization before adding folders.
              </div>
            )}
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                type="submit"
                disabled={loading || organizations.length === 0 || !initialOrgId}
